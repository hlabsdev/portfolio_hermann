    ...
    <div class="relative">
      <div class="overflow-hidden">
        <div class="flex transition-transform duration-500" style={"transform: translateX(-#{@current_slide * 100}%)"}>
          <%= for project <- @projects do %>            <div class={"carousel-item flex-none w-full #{if @current_slide == project.id, do: "active"}"}>
      ...
      <button
        ...
        class="absolute left-4 top-1/2 -translate-y-1/2 bg-white/50 dark:bg-gray-800/50 p-2 rounded-full"
      >←</button>
      <button
        phx-click={JS.push("next-slide")}
        class="absolute right-4 top-1/2 -translate-y-1/2 bg-white/50 dark:bg-gray-800/50 p-2 rounded-full"
      >→</button>
    </div>
    """
  end
